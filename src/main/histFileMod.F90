...
! PRIVATE MEMBER FUNCTIONS:
  ...
  private :: update_hbuf_field         ! Updates history buffer for specific field and tape
...
!
  type field_info
     ...
     character(len=8) :: type1d                ! 1d history buffer type ["gridcell","landunit","column","pft","roflnd","rofocn"]
     character(len=8) :: type2d                ! second dimension type ["levsoi","levlak","numrad","subname(n)"]
     character(len=8) :: typexy                ! xy grid type ["clm", "rof"]
     character(len=8) :: type1d_out            ! 1d output type ["gridcell","landunit","column","pft","roflnd","rofocn"]
     ...
     integer :: num1d                          ! size of first dimension (overall all nodes)
...
!-----------------------------------------------------------------------
    ...
    do t = 1,ntapes
       do f = 1,tape(t)%nflds
          ...
          allocate (tape(t)%hlist(f)%hbuf(beg1d:end1d,num2d))
          tape(t)%hlist(f)%hbuf(:,:) = 0._r8
          allocate (tape(t)%hlist(f)%nacs(beg1d:end1d,num2d))
...
! LOCAL VARIABLES:
    ...
    character(len=8) :: type1d      ! history buffer 1d type
    integer :: index                ! index
    integer :: begp, endp           ! per-proc beginning and ending pft indices
    integer :: begc, endc           ! per-proc beginning and ending column indices
    integer :: begl, endl           ! per-proc beginning and ending landunit indices
...
!-----------------------------------------------------------------------
    ...
    if (hist_dov2xy(t)) then
       ...
       type1d = tape(t)%hlist(n)%field%type1d_out
       ...
       ! Override the 1d output type based on namelist setting of  hist_type1d_pertape
...
!CSD$ PARALLEL DO PRIVATE (f)
       do f = 1,tape(t)%nflds
          call update_hbuf_field (t,f)
       end do
!CSD$ END PARALLEL DO
!$OMP END PARALLEL DO
    end do
...
! LOCAL VARIABLES:
    ...
    integer :: j                   ! 2d index
    integer :: beg,end             ! beginning and ending indices
    integer :: num2d               ! size of second dimension (e.g. number of vertical levels)
    character(len=1)  :: avgflag   ! time averaging flag
...
!-----------------------------------------------------------------------
    ...
    beg     =  tape(t)%hlist(f)%field%beg1d
    end     =  tape(t)%hlist(f)%field%end1d
    num2d   =  tape(t)%hlist(f)%field%num2d
...
!cdir nodep
          do k = beg,end
             if (clmptr_rs(hpindex)%ptr(k)/= spval) then
                hbuf(k,1) = clmptr_rs(hpindex)%ptr(k)
             else
                hbuf(k,1) = spval
             endif
             nacs(k,1) = 1
...
!cdir nodep
             do k = beg,end
                if (clmptr_ra(hpindex)%ptr(k,j) /= spval) then
                   hbuf(k,j) = clmptr_ra(hpindex)%ptr(k,j)
                else
                   hbuf(k,j) = spval
                endif
                nacs(k,j) = 1
...
!cdir nodep
          do k = beg,end
             if (clmptr_rs(hpindex)%ptr(k) /= spval) then
                ...
                hbuf(k,1) = hbuf(k,1) + clmptr_rs(hpindex)%ptr(k)
                nacs(k,1) = nacs(k,1) + 1
             else
...
!cdir nodep
             do k = beg,end
                if (clmptr_ra(hpindex)%ptr(k,j) /= spval) then
                   if (nacs(k,j) == 0) hbuf(k,j) = 0.
                   hbuf(k,j) = hbuf(k,j) + clmptr_ra(hpindex)%ptr(k,j)
                   nacs(k,j) = nacs(k,j) + 1
                else
...
!cdir nodep
          do k = beg,end
             if (clmptr_rs(hpindex)%ptr(k) /= spval) then
                ...
                hbuf(k,1) = max( hbuf(k,1), clmptr_rs(hpindex)%ptr(k) )
             else
                hbuf(k,1) = spval
             endif
             nacs(k,1) = 1
...
!cdir nodep
             do k = beg,end
                if (clmptr_ra(hpindex)%ptr(k,j) /= spval) then
                   if (nacs(k,j) == 0) hbuf(k,j) = -1.e50
                   hbuf(k,j) = max( hbuf(k,j), clmptr_ra(hpindex)%ptr(k,j) )
                else
                   hbuf(k,j) = spval
                end if
                nacs(k,j) = 1
...
!cdir nodep
          do k = beg,end
             if (clmptr_rs(hpindex)%ptr(k) /= spval) then
                if (nacs(k,1) == 0) hbuf(k,1) = +1.e50
                hbuf(k,1) = min( hbuf(k,1), clmptr_rs(hpindex)%ptr(k) )
             else
                hbuf(k,1) = spval
             endif
             nacs(k,1) = 1
...
!cdir nodep
             do k = beg,end
                if (clmptr_ra(hpindex)%ptr(k,j) /= spval) then
                   if (nacs(k,j) == 0) hbuf(k,j)= +1.e50
                   hbuf(k,j) = min( hbuf(k,j), clmptr_ra(hpindex)%ptr(k,j) )
                else
                   hbuf(k,j) = spval
                endif
                nacs(k,j) = 1
    ...
    case default
       ...
       write(6,*)'UPDATE_HBUF_FIELD error: invalid time averaging flag ', avgflag
       call endrun()

    end select

  end subroutine update_hbuf_field
...
! !USES:
    use clmtype
    use ncdio
    use decompMod   , only : get_proc_global, map_dc2sn, get_sn_land1d, get_sn_cols1d, get_sn_pfts1d
...
!-----------------------------------------------------------------------
    ...
    str = '$Name: clm3_deva_21 $'
    call check_ret(&
         nf_put_att_text (nfid(t), NF_GLOBAL, 'version', len_trim(str), trim(str)), subname)

    str = '$Id: histFileMod.F90,v 1.19.4.48 2004/06/21 16:01:30 mvertens Exp $'
    call check_ret(&
...
!cdir nodep
          ...
          if (tape(t)%dov2xy) then
             ...
             call ncd_iolocal(varname=varname, datadim1='gridcell', datadim2='levsoi', data=histo, &
                  ncid=nfid(t), nlonxy=lsmlon, nlatxy=lsmlat, flag='write')
          else
             call ncd_iolocal(varname=varname, datadim1='column'  , datadim2='levsoi', data=histi, &
...
! LOCAL VARIABLES:
    ...
    character(len=max_chars) :: units    ! units
    character(len=max_namlen):: varname  ! variable name
    character(len= 8) :: typexy          ! 2d output type
    ...
    character(len=8)  :: p2c_scale_type  ! history output 1d scaling type
    character(len=8)  :: c2l_scale_type  ! history output 1d scaling type
    character(len=8)  :: l2g_scale_type  ! history output 1d scaling type
...
!-----------------------------------------------------------------------
    ...
    do f = 1,tape(t)%nflds
       ...
       typexy         = tape(t)%hlist(f)%field%typexy
       ...
       if (mode == 'define') then
          ...
          if      (type1d_out == nameg) then
             ...
             write(6,*)'HFIELDS_WRITE error: incorrect value of type1d_out= ',type1d_out
             call endrun()
          end if

          ! Determine output buffer

          if (type1d_out == type1d) then

             histo => tape(t)%hlist(f)%hbuf

          else

             histi => tape(t)%hlist(f)%hbuf
             allocate(histo(beg1d_out:end1d_out,num2d), stat=ier)
             if (ier /= 0) then
                write(6,*)'HFIELDS_WRITE error: allocation error'; call endrun()
             end if
             histo(beg1d_out:end1d_out,1:num2d) = spval

             if      (type1d == namep .and. type1d_out == nameg) then

                call p2g(begp, endp, begc, endc, begl, endl, begg, endg, num2d, histi, histo, &
                     p2c_scale_type, c2l_scale_type, l2g_scale_type)

             else if (type1d == namep .and. type1d_out == namel) then

                call p2l(begp, endp, begc, endc, begl, endl, num2d, histi, histo, &
                     p2c_scale_type, c2l_scale_type)

             else if (type1d == namep .and. type1d_out == namec) then

                call p2c(begp, endp, begc, endc, num2d, histi, histo, &
                     p2c_scale_type)

             else if (type1d == namec .and. type1d_out == nameg) then

                call c2g(begc, endc, begl, endl, begg, endg, num2d, histi, histo, &
                     c2l_scale_type, l2g_scale_type)

             else if (type1d == namec .and. type1d_out == namel) then

                call c2l(begc, endc, begl, endl, num2d, histi, histo, &
                     c2l_scale_type)

             else if (type1d == namel .and. type1d_out == nameg) then

                call l2g(begl, endl, begg, endg, num2d, histi, histo, &
                     l2g_scale_type)
          ...
          if (type1d_out == 'lndrof' .or. type1d_out == 'ocnrof') then
             ...
             call ncd_iolocal(flag='write', varname=varname, datadim1=type1d_out, data=hist1do, &
                  ncid=nfid(t), nlonxy=nlonxy, nlatxy=nlatxy, nt=nt)
             ...
             if (num2d == 1) then
                ...
                call ncd_iolocal(flag='write', varname=varname, datadim1=type1d_out, datadim2=type2d, data=histo, &
                     ncid=nfid(t), nlonxy=nlonxy, nlatxy=nlatxy, nt=nt)
             end if
          ...
          if (num2d == 1) deallocate(hist1do)
          if (type1d_out /= type1d) deallocate(histo)
...
! !USES:
    ...
    use clmtype  , only : nameg, namel, namec, namep
    use decompMod, only : get_proc_bounds, get_proc_global, &
                          get_sn_land1d, get_sn_cols1d, get_sn_pfts1d, map_dc2sn
!
! !ARGUMENTS:
    implicit none
    integer, intent(in) :: t                ! tape index
...
!-----------------------------------------------------------------------
    ...
    if (mode == 'define') then
       ...
       call ncd_iolocal(varname='grid1d_lon', data=gptr%londeg, datadim1='gridcell', ncid=ncid, flag='write')
       call ncd_iolocal(varname='grid1d_lat', data=gptr%latdeg, datadim1='gridcell', ncid=ncid, flag='write')
       call ncd_iolocal(varname='grid1d_ixy', data=gptr%ixy   , datadim1='gridcell', ncid=ncid, flag='write')
       call ncd_iolocal(varname='grid1d_jxy', data=gptr%jxy   , datadim1='gridcell', ncid=ncid, flag='write')

       ! Write landunit info

       call ncd_iolocal(varname='land1d_lon', data=lptr%londeg, datadim1='landunit', ncid=ncid, flag='write')
       call ncd_iolocal(varname='land1d_lat', data=lptr%latdeg, datadim1='landunit', ncid=ncid, flag='write')
       call ncd_iolocal(varname='land1d_ixy', data=lptr%ixy   , datadim1='landunit', ncid=ncid, flag='write')
       call ncd_iolocal(varname='land1d_jxy', data=lptr%jxy   , datadim1='landunit', ncid=ncid, flag='write')
       if (masterproc) then
          call get_sn_land1d(iltemp, type1d='gridcell')
          call ncd_ioglobal(varname='land1d_gi', data=iltemp, ncid=ncid, flag='write')
       end if
       call ncd_iolocal(varname='land1d_wtgcell'  , data=lptr%wtgcell, datadim1='landunit', ncid=ncid, flag='write')
       call ncd_iolocal(varname='land1d_ityplunit', data=lptr%itype  , datadim1='landunit', ncid=ncid, flag='write')

       ! Write column info

       call ncd_iolocal(varname='cols1d_lon', data=cptr%londeg, datadim1='column', ncid=ncid, flag='write')
       call ncd_iolocal(varname='cols1d_lat', data=cptr%latdeg, datadim1='column', ncid=ncid, flag='write')
       call ncd_iolocal(varname='cols1d_ixy', data=cptr%ixy   , datadim1='column', ncid=ncid, flag='write')
       call ncd_iolocal(varname='cols1d_jxy', data=cptr%jxy   , datadim1='column', ncid=ncid, flag='write')
       if (masterproc) then
       ...
       call ncd_iolocal(varname='cols1d_wtgcell', data=cptr%wtgcell, datadim1='column', ncid=ncid, flag='write')
       call ncd_iolocal(varname='cols1d_wtlunit', data=cptr%wtlunit, datadim1='column', ncid=ncid, flag='write')
       if (masterproc) then
          do c = 1,numc
             l = cptr%landunit(c)
       ...
       call ncd_iolocal(varname='pfts1d_lon', data=pptr%londeg, datadim1='pft', ncid=ncid, flag='write')
       call ncd_iolocal(varname='pfts1d_lat', data=pptr%latdeg, datadim1='pft', ncid=ncid, flag='write')
       call ncd_iolocal(varname='pfts1d_ixy', data=pptr%ixy   , datadim1='pft', ncid=ncid, flag='write')
       call ncd_iolocal(varname='pfts1d_jxy', data=pptr%jxy   , datadim1='pft', ncid=ncid, flag='write')
       if (masterproc) then
       ...
       call ncd_iolocal(varname='pfts1d_wtgcell'  , data=pptr%wtgcell, datadim1='pft', ncid=ncid, flag='write')
       call ncd_iolocal(varname='pfts1d_wtlunit'  , data=pptr%wtlunit, datadim1='pft', ncid=ncid, flag='write')
       call ncd_iolocal(varname='pfts1d_wtcol'    , data=pptr%wtcol  , datadim1='pft', ncid=ncid, flag='write')
       call ncd_iolocal(varname='pfts1d_itype_veg', data=pptr%itype  , datadim1='pft', ncid=ncid, flag='write')
       if (masterproc) then
          do p = 1,nump
             l = pptr%landunit(p)
...
!------------------------------------------------------------------------
    ...
    if (flag == 'read') then
     ...
     if (masterproc) then
        ...
        if (flag == 'write') then
           varid(:,:) = -999. ! this is done for backwards compatibility of restarts
        ...
        do t = 1,ntapes
           ...
           do f=1,tape(t)%nflds
              ...
              if (ier /= 0) then
                 write(6,*)'RESTART_HISTORY error: read/write error 3: End or error condition writing ', &
     ...
     if (flag == 'read') then
        do t = 1,ntapes
           do f = 1,tape(t)%nflds
              ...
              allocate (tape(t)%hlist(f)%hbuf(beg1d:end1d,num2d), &
                        tape(t)%hlist(f)%nacs(beg1d:end1d,num2d), stat=ier)
              if (ier /= 0) then
...
!cdir nodep
                 ...
                 allocate(ibuf2d(num2d,num1d))
                 allocate(rbuf2d(num2d,num1d))
                 if (flag == 'read') then
